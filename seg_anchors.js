...
async function getLinePolyline(city, stations) {
    ...
    for (const seg of (transit.segments || [])) {
        for (const bl of (seg.bus?.buslines || [])) {
            if (bl.polyline && bl.type && (bl.type.includes("\u5730\u94C1") || bl.type.includes("\u5730\u94C1") ||
                 bl.type.includes("\u5730\u94C1") || bl.type.includes("\u5730\u94C1") || bl.type.includes("\u5730\u94C1") ||
                 bl.type.includes("\u5730\u94C1") || bl.type.includes("\u5730\u94C1"))) {
                if (!polyline) { polyline = bl.polyline; }
            }
        }
    }
...
async function processLine(l, cityName, idx, total) {
    ...
    if (!polyline || polyline.length < 2) {
        const isAirport = l.id.includes("CA") || l.id.includes("JX") || l.id.includes("JC") || l.name.includes("\u5730\u94C1");
        if (isAirport) {
    ...
    if (firstDist > 5000 || lastDist > 5000) {
        const elapsed = Date.now() - startTime;
...
if (!slug) { console.log("Usage: seg_anchors.js {slug} {\u5730\u94C1?}"); process.exit(1); }
